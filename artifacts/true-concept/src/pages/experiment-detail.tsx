...
export default function ExperimentDetailPage() {
  ...
  const diff = experiment ? diffBadge[experiment.difficulty] : null;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 pb-28 md:pb-8 blob-bg">
      ...
      <div className="relative overflow-hidden liquid-dark rounded-3xl p-6 text-white mb-5">
        ...
        <div className="relative">
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            ...
            {diff && (
              <span className="text-xs font-black px-3 py-1.5 rounded-full" style={diff.style}>{diff.text}</span>
            )}
          </div>
      ...
      {/* Objective */}
      <div className="liquid-card rounded-3xl p-5 mb-4">
        <h2 className="font-black text-gray-900 mb-2 flex items-center gap-2">🎯 Objective</h2>
        <p className="text-sm text-gray-700 font-medium leading-relaxed">{experiment.objective}</p>
      </div>

      {/* Simulation */}
      {SimComponent && (
        <div className="liquid-panel rounded-3xl p-5 mb-4">
          <h2 className="font-black text-gray-900 mb-4 flex items-center gap-2">⚗️ Interactive Simulation</h2>
          <Suspense fallback={
      ...
      {/* Procedure */}
      <div className="liquid-card rounded-3xl p-5 mb-4">
        <h2 className="font-black text-gray-900 mb-4 flex items-center gap-2">📋 Procedure</h2>
        <ProcedureList text={experiment.procedure} />
      </div>

      {/* Expected Result */}
      <div className="liquid-card rounded-3xl p-5 mb-4" style={{ background: "rgba(16,185,129,0.08)" }}>
        <h2 className="font-black text-emerald-800 mb-2 flex items-center gap-2">✅ Expected Result</h2>
        <p className="text-sm text-emerald-800/90 font-medium leading-relaxed">{experiment.expectedResult}</p>
      </div>

      {/* Explanation */}
      <div className="liquid-card rounded-3xl p-5" style={{ background: "rgba(59,130,246,0.07)" }}>
        <h2 className="font-black text-blue-800 mb-2 flex items-center gap-2">💡 Scientific Explanation</h2>
